...
<script lang="ts">
	...
	const POS_CLASS: Record<string, string> = {
		start: 'w-5/6 sm:w-5/12 bg-gray-100 p-6',
		center: 'w-5/6 sm:w-1/2 bg-gray-100 p-6',
		end: 'w-5/6 sm:w-5/12 bg-gray-100 p-6'
	};
	...
	onMount(async () => {
		...
		const uniq = new SvelteMap<string, Media>();
...
{#if resolvedSteps.length}
    ...
    -->
		<div
			...
			style={`height: 100vh; width: 100vw;${released ? ` top: ${releaseTopPx}px;` : ''}`}
		>
			{#if topMedia}
				<div class="layer layer-top" aria-hidden="true">
					{#if topMedia.kind === 'image'}
						<div
							class="media image"
							style={`background-image:url("${encodeURI(topMedia.url)}")`}
						></div>
					{:else}
						<video
			...
			{#if bottomMedia}
				<div class={'layer layer-bottom ' + (bottomShow ? 'show' : '')} aria-hidden="true">
					{#if bottomMedia.kind === 'image'}
						<div
							class="media image"
							style={`background-image:url("${encodeURI(bottomMedia.url)}")`}
						></div>
					{:else}
						<video
							class="media video"
		...
		<div class="scrolly-steps">
			{#each resolvedSteps as step, i (i)}
				<div class="step" bind:this={stepEls[i]} style={`min-height:${stepHeightVh}vh;`}>
					<div class="w-full">
						<div
							class="step-row flex flex-wrap items-start md:px-6 lg:px-12"
